...
@section('titulo','Atención a la Demanda')
...
@include('errors.errores')
	...
	<!-- BUSCADORES/FILTROS -->
	<div class="row">
		<form action="{{route('demanda_index')}}" method="GET">
		...
		<div class="panel panel-default">
			<table class="table table-hover">
				...
				<tbody>
					@if(count($clientes) < 1)
						<tr>
							<td colspan="4" class="text-center">NO SE ENCONTRO NINGUN REGISTRO</td>
						</tr>
					@endif
					...
					<tr>
						...
						<td>
							{{
								$cl->calle->nombre . ", " . $cl->calle->colonia->nombre. " #" .$cl->num_ext . $cl->num_int
							}}
						</td>
						<!-- <td>{{ $cl->estado_civil }}</td>
						<td>{{ $cl->ocupacion->nombre }}</td> -->
						<td class="text-center"><a href="{{route('demanda',$cl->id_cliente)}}" class="btn btn-xs BtnOrange"><span class="icon icon-play3"></span> Iniciar</a></td>
					</tr>
					@endforeach
				</tbody>
			</table>
